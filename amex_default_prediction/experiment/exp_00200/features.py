...
def main():
    ...
    convert_cols = get_low_nunique_cols(
        train.select_dtypes(include=["float32"]), threshold=20
    )
    ...
    with open("./output/convert_cols.pkl", "wb") as file:
        pickle.dump(convert_cols, file)
    ...
    test = convert_float_to_int(test, convert_cols)
    print(test.head())
