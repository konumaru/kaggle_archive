...
class StackedNMultiHeadAttention(nn.Module):
    def __init__(self, n_stacks, n_dims, n_heads, seq_len, n_multihead=1, dropout=0.0):
        ...
        self.mask = torch.triu(torch.ones(seq_len, seq_len), diagonal=1).to(
            dtype=torch.bool
        )
    ...
    def forward(self, input_q, input_k, input_v, encoder_output=None, break_layer=None):
        for stack in range(self.n_stacks):
            for multihead in range(self.n_multihead):
                ...
                heads_output, _ = self.multihead_layers[stack][multihead](
                ...
                if encoder_output != None and multihead == break_layer:
                    ...
                    ), " break layer should be less than multihead layers and postive integer"
...
class PlusSAINTModule(pl.LightningModule):
    ...
    def training_epoch_end(self, training_ouput):
        out = np.concatenate(
            [i["outs"].cpu().detach().numpy() for i in training_ouput]
        ).reshape(-1)
        labels = np.concatenate(
            [i["labels"].cpu().detach().numpy() for i in training_ouput]
        ).reshape(-1)
    ...
    def validation_epoch_end(self, validation_ouput):
        out = np.concatenate(
            [i["outs"].cpu().detach().numpy() for i in validation_ouput]
        ).reshape(-1)
        labels = np.concatenate(
            [i["labels"].cpu().detach().numpy() for i in validation_ouput]
        ).reshape(-1)
        auc = roc_auc_score(labels, out)
...
if __name__ == "__main__":
    ...
    trainer = pl.Trainer(
        gpus=4, max_epochs=5, progress_bar_refresh_rate=21, accelerator="ddp"
    )
    trainer.fit(
        model=saint_plus,
