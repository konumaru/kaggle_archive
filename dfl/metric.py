...
def event_detection_ap(
    ...
    assert_index_equal(
        submission.columns, pd.Index(["video_id", "time", "event", "score"])
    )
    ...
    detections_grouped = aggregation_keys.merge(
        detections_filtered, on=["event", "video_id"], how="left"
    ).groupby(["event", "tolerance", "video_id"])
    ground_truths_grouped = aggregation_keys.merge(
        ground_truths, on=["event", "video_id"], how="left"
    ).groupby(["event", "tolerance", "video_id"])
    ...
    mean_ap = ap_table.groupby("event").mean().sum() / 3
